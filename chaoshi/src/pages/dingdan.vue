    ...
    <div class="container">
      ...
      :data=" data"   >
        ...
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="setRow(scope.row,1)">已发货</el-button>
            <el-button type="success" size="mini"  @click="setRow(scope.row,2)">已送达</el-button>
          </template>
        </el-table-column>   
      </el-table>
      <!-- 分页实现 -->
...
//start设定新数组的起始位置，end设定新数组的结束位置。
  ...
  methods: {
    menuClick(row){
      this.$router.push({
        path: 'dd',
        query:{ddid:row.id,name:row.username},
    ...
    getData () {
      this.tableData = []
      this.tempData = []
      axios.post('api/chaoshi/getdingdan').then(res => {        
        console.log(res);
